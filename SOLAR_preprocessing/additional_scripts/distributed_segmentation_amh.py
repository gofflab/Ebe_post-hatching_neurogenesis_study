...
def numpy_array_to_zarr(write_path: Path | str, array, chunks):
    ...
    zarr_array = zarr.create(
        ...
        chunks=chunks,
        dtype=array.dtype,
        overwrite=True,  # v3 uses overwrite
    )
    zarr_array[...] = array
    return zarr_array


def get_zarr_store_path(zarr_array):
    """
    Extract file system path from a Zarr array, compatible with Zarr v3

    Parameters
    ----------
    zarr_array : zarr.Array
        The Zarr array to extract path from

    Returns
    -------
    Path
        The file system path to the Zarr store
    """
    ...
    # Zarr v2 FSStore fallback
    if hasattr(store, "path"):
        return Path(store.path)

    # Extract from string representation as fallback
    store_str = str(store)
    if "LocalStore" in store_str:
        import re

        match = re.search(r"LocalStore\('([^']+)'\)", store_str)
        if match:
...
def distributed_eval(
    ...
    store_path = get_zarr_store_path(input_zarr)
    worker_logs_dir = store_path.parent / worker_logs_dirname
    ...
    def check_block_has_data(crop, zarr_array, channels_list, threshold=0) -> bool:
        """
        Checks if a given crop in a Zarr array contains any data above a threshold
        in the specified channels only.
        Returns True if data exists in the relevant channels, False otherwise.
        """
        try:
            data_slice = zarr_array[crop]
            # channels_list is 1-indexed from Cellpose, convert to 0-indexed for zarr
            channel_indices = [c - 1 for c in channels_list]
            relevant_channels = data_slice[..., channel_indices]

            if threshold == 0:
                return relevant_channels.any()
            else:
                return (relevant_channels > threshold).any()
    ...
    try:
        ...
        check_futures = cluster.client.map(
            ...
            channels_list=eval_kwargs["channels"],  # Pass the channels list
            threshold=0,
        )
...
def adjacent_faces(block_indices, faces):
    ...
    for block_index in block_indices:
        # Only iterate over spatial dimensions (exclude channel axis)
        spatial_dims = min(3, len(block_index))  # Ensure we don't go beyond Z,Y,X

        for ax in range(spatial_dims):  # Was: range(len(block_index)) - included channel!
            ...
            try:
                ...
                # SAFETY CHECK: Ensure faces have compatible shapes
                if a.shape != b.shape:
                    print(f"Warning: Face shape mismatch at axis {ax}: {a.shape} vs {b.shape}")
                    continue

                face_pairs.append(np.concatenate((a, b), axis=ax))
            except (KeyError, ValueError, IndexError) as e:
                # More specific error handling without crashing
                continue
    return face_pairs
...
def block_face_adjacency_graph(faces, nlabels):
    ...
    for face in faces:
        # Find the axis where face was concatenated (size == 2)
        concat_axis = None
        for i, size in enumerate(face.shape):
            if size == 2:
                concat_axis = i
                break

        if concat_axis is None:
            print(f"Warning: Could not find concatenation axis in face shape {face.shape}")
            continue

        try:
            # Split the concatenated face
            sl0 = tuple(slice(0, 1) if i == concat_axis else slice(None) for i in range(face.ndim))
            sl1 = tuple(slice(1, 2) if i == concat_axis else slice(None) for i in range(face.ndim))

            a = shrink_labels(face[sl0], 1.0)
            b = shrink_labels(face[sl1], 1.0)

            # Recombine along same axis
            face_processed = np.concatenate((a, b), axis=concat_axis)

            # Process with dask_image
            mapped = dask_image.ndmeasure._utils._label._across_block_label_grouping(
                face_processed, structure
            )
    ...
    try:
        ...
        print(f"Warning: Failed to concatenate mappings: {e}")
        return scipy.sparse.identity(nlabels + 1, format="csr")
...
def main(
    ...
    try:
        # Find the actual indices of the requested channels
        channel_indices = [key.index(c) for c in channels.split(",")]
        # Convert to 1-based indexing as expected by Cellpose
        channels_list = [idx + 1 for idx in channel_indices]
        logger.info(f"Using channels: {channels}")
        logger.info(f"Channel mapping: {dict(zip(channels.split(','), channel_indices))}")
        logger.info(f"Cellpose channels_list: {channels_list}")
    except ValueError:
        raise ValueError(f"Channel names {channels} not found in {key}")

    processing_blocksize = (input_zarr_array.shape[0], 512, 512, input_zarr_array.shape[3])
    try:
        if overwrite:
    ...
    cellpose_eval_kwargs = {
        ...
        "normalize": {},
        "flow_threshold": 0.9,  # Useless in 3D
        ...
        "z_axis": 0,
        "channel_axis": 3,
        "niter": 2000,  # test at a higher iteration for longer cells
    }
